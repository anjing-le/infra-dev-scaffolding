...
- `RemoteHttpClient`、`ServiceEndpointResolver`、`RemoteCallerResolver`、`RemoteCallPolicy`、`RemoteCallObserver`、`RemoteCallWrapper`：远程调用、服务地址解析、调用方身份解析、调用治理策略、调用审计观察与重试适配。
