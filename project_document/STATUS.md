...
- 验证后回补的关键契约：
  ...
  - `node scripts/check-remote-http-contract.js` 已校验后端远程 HTTP 调用具备服务解析扩展点、服务发现/地址注册表扩展点、配置型地址表、调用方身份解析扩展点、调用治理策略扩展点、调用审计扩展点、泛型响应重载、示例不再拼接本地绝对 URL、文档使用 `serviceId + path`，且服务间透传头来自 platform contract。
