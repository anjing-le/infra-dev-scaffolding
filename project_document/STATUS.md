...
| S5 分布式可观测基线 | In progress | 已落地 MDC 日志字段、Controller 路径/耗时/错误码访问日志、异步上下文传播、`RemoteCallWrapper.serviceCallHeaders(callerId)`、支持 `ServiceEndpointResolver`、`RemoteCallerResolver`、`RemoteCallPolicy`、`RemoteCallObserver` 和 `ParameterizedTypeReference` 的 `RemoteHttpClient`、错误码分段指南、`scripts/check-backend-context-contract.js`、`scripts/check-async-context-contract.js`、`scripts/check-error-codes.js`、`/api/test/features` 可选能力状态接口和 dev/test/prod profile 矩阵 |
| S7 契约生成与共享包 | In progress | 已落地 `project_document/SHARED_KERNEL_GUIDE.md`、`project_document/OPENAPI_CONTRACT_GUIDE.md`、`project_document/SERVICE_BOUNDARY_GUIDE.md`、`scripts/generate-openapi-frontend-types.js`、`scripts/check-openapi-contract.js`、`scripts/check-service-boundaries.js`、`scripts/check-shared-kernel.js` 和 `scripts/check-async-context-contract.js`，先守住未来可抽 `anjing-common`、服务边界、异步上下文传播与前端类型生成的契约边界 |
...
- 验证后回补的关键契约：
  ...
  - `node scripts/generate-openapi-frontend-types.js` 已从真实 OpenAPI JSON 生成 `frontend/src/contracts/openapi/schemas.ts`，`frontend/src/api/model/authModel.ts` 已从生成的 `LoginRequest`、`AuthTokenResponse`、`CurrentUserResponse` 和 `RefreshTokenRequest` 派生类型。
  - `node scripts/check-openapi-runtime-contract.js` 已读取真实 OpenAPI JSON、`contracts/service-boundaries.json` 和 `contracts/platform-contract.json`，校验 openapi 运行 route/method 全量存在，并由 `./scripts/probe-backend-dev.sh` 自动执行；`probe-backend-dev.sh` 同时执行 `node scripts/generate-openapi-frontend-types.js <openapi.json> --check` 防止前端生成 schema 过期。
  ...
  - HTTP 服务间调用使用 `RemoteHttpClient`，内部服务地址通过 `ServiceEndpointResolver` 统一解析，默认 `ConfiguredServiceEndpointResolver` 读取 `app.remote-http.service-base-urls`，调用方身份通过 `RemoteCallerResolver` 统一解析，默认 `DefaultRemoteCallerResolver` 支持请求级覆盖、配置默认值和应用 id 回退，调用治理通过 `RemoteCallPolicy` 预留熔断、限流、白名单和灰度策略挂点，调用审计通过 `RemoteCallObserver` 预留审计表、指标和 tracing 挂点，调用侧优先使用 `serviceId + path`；标准响应、分页响应或列表响应使用 `ParameterizedTypeReference<T>` 保留嵌套泛型；自定义 adapter 使用 `RemoteCallWrapper.serviceCallHeaders(callerId)` 按 `backendPropagatedHeaders` 透传 requestId、traceId、租户、用户、语言和时区。
  - `node scripts/check-remote-http-contract.js` 已校验后端远程 HTTP 调用具备服务解析扩展点、配置型地址表、调用方身份解析扩展点、调用治理策略扩展点、调用审计扩展点、泛型响应重载、示例不再拼接本地绝对 URL、文档使用 `serviceId + path`，且服务间透传头来自 platform contract。
  ...
  - `node scripts/check-openapi-contract.js` 已校验 springdoc 依赖、OpenAPI 配置、平台请求头、Auth 明确 DTO/VO、前端生成 schema 和前端 auth 类型派生关系。
