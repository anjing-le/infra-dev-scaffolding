...
- 验证后回补的关键契约：
  ...
  - HTTP 服务间调用使用 `RemoteHttpClient`，内部服务地址通过 `ServiceEndpointResolver` 统一解析，默认 `ConfiguredServiceEndpointResolver` 读取 `app.remote-http.service-base-urls`，调用侧优先使用 `serviceId + path`；标准响应、分页响应或列表响应使用 `ParameterizedTypeReference<T>` 保留嵌套泛型；自定义 adapter 使用 `RemoteCallWrapper.serviceCallHeaders(callerId)` 按 `backendPropagatedHeaders` 透传 requestId、traceId、租户、用户、语言和时区。
  - `node scripts/check-remote-http-contract.js` 已校验后端远程 HTTP 调用具备服务解析扩展点、配置型地址表、泛型响应重载、示例不再拼接本地绝对 URL、文档使用 `serviceId + path`，且服务间透传头来自 platform contract。
