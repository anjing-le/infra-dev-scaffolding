...
| S1 工程母版收口 | Ready | `project_document/PROJECT_CONSTRAINTS.md`、`project_document/NEW_MODULE_GUIDE.md`、`project_document/UI_DESIGN_GUIDE.md`、`project_document/DEMO_EVIDENCE.md`、`project_document/ci/quality-gate.yml`、`./scripts/check-template.sh`、`./scripts/smoke-copy.sh` 和 `./scripts/quality-gate.sh` 已纳入门禁 |
...
- 验证后回补的关键契约：
  ...
  - `node scripts/check-scaffold-governance.js` 已校验 CI 工作流、根入口文档、新模块指南、UI 设计基线、演示证据、全局玻璃 token、登录页玻璃/虚线入口和前端 legacy API 使用边界。
