...
- 统一请求上下文：请求进入后生成或透传 `X-Request-Id`，保留 `X-Trace-Id`、`X-Tenant-Id`、`X-User-Id`、`X-Time-Zone`、`Accept-Language`，并把语言归一化到 platform contract 支持列表。
...
4. 建立请求上下文。
   ...
   后端通过 filter/interceptor 统一读取和生成 requestId、traceId、locale、timeZone，其中 locale 由 `LocaleUtils` 按 platform contract 归一化。前端请求拦截器透传语言和时区。
