...
- 请求上下文已开始具备 `requestId`、会话级 `traceId`、语言和时区透传，并已接入日志格式、远程调用请求头生成、前端 `HttpError` 链路上下文和统一 HTTP client adapter；前端上下文头已由 `scripts/check-frontend-context-contract.js` 守护，后续需要继续接入权限上下文和真实 RPC client adapter。
