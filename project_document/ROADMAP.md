...
- 远程调用 wrapper 和 HTTP client adapter 支持上下文透传、调用方身份、超时、重试、审计字段和标准/分页/列表响应的泛型反序列化。
...
- `RemoteHttpClient` 已支持通过 `serviceId + path` 调用内部服务，服务 base URL 统一配置在 `app.remote-http.service-base-urls`，并通过 `ParameterizedTypeReference<T>` 保留嵌套泛型响应；`RemoteCallWrapper` 已按 `backendPropagatedHeaders` 透传服务间上下文，`scripts/check-remote-http-contract.js` 已纳入守卫。
