...
- `RemoteHttpClient` 已支持通过 `serviceId + path` 调用内部服务，服务 base URL 统一配置在 `app.remote-http.service-base-urls`；`RemoteCallWrapper` 已按 `backendPropagatedHeaders` 透传服务间上下文，`scripts/check-remote-http-contract.js` 已纳入守卫。
