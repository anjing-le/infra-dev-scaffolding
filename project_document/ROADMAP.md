...
验收：
- OpenAPI schema 和 operation 生成前端 types，后续评估继续生成完整 API client。
- 评估抽出共享包，只放稳定工具和类型。
...
- `springdoc-openapi-starter-webmvc-api` 已提供 `/v3/api-docs`，`scripts/generate-openapi-frontend-types.js` 已生成 `frontend/src/contracts/openapi/schemas.ts` 与 `frontend/src/contracts/openapi/operations.ts`，`scripts/check-openapi-contract.js`、`scripts/check-openapi-runtime-contract.js` 与 `./scripts/probe-backend-dev.sh` 已校验 OpenAPI 配置、service boundary 运行路径、平台请求头、前端生成 schema / operation 类型和 auth DTO/VO 契约。
